...
$sqlh4 = "SELECT time from db_rep WHERE date > ( NOW() - INTERVAL '1 hour') and server = 'venus' order by date;";
$sqlh3 = "SELECT time from db_rep WHERE date > ( NOW() - INTERVAL '1 hour') and server = 'Saturn' order by date;";
$sqlh2 = "SELECT time from db_rep WHERE date > ( NOW() - INTERVAL '1 hour') and server = 'jupiter' order by date;";
//$sqlh1 = "SELECT time from db_rep WHERE date > ( NOW() - INTERVAL '1 hour') and server = 'neptune' order by date;";
$sqlavg = "SELECT server, round(avg(time)::numeric,3) as avgtime from db_rep WHERE prc = 'backup' and date > ( NOW() - INTERVAL '2 hour') and server = 'neptune' group by server;";
